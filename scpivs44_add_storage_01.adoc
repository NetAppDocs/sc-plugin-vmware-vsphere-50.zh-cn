...
| 存储系统 | 输入存储集群或Storage VM的FQDN或管理LIF的IP地址。SnapCenter VMware插件不支持在不同集群上使用多个同名Storage VM。 
...
| 超时 | 输入SnapCenter VMware插件在超时操作之前应等待的秒数。默认值为60秒。 


| 首选 IP | 如果此Storage VM具有多个管理IP地址、请选中此框并输入您希望SnapCenter VMware插件使用的IP地址。* 注： * 输入 IP 地址时，请勿使用方括号（）。 
