...
keywords:  
summary: 如果您不再需要SnapCenter数据保护功能、则必须更改SnapCenter VMware插件的配置。 例如、如果您在测试环境中部署了此插件、则可能需要在该环境中禁用SnapCenter功能、然后在生产环境中启用这些功能。 
---
...
[role="lead"]
如果您不再需要SnapCenter数据保护功能、则必须更改SnapCenter VMware插件的配置。 例如、如果您在测试环境中部署了此插件、则可能需要在该环境中禁用SnapCenter功能、然后在生产环境中启用这些功能。
...
... 登录到 VMware vSphere 。
... 关闭虚拟机。
... 在左侧导航屏幕中，右键单击SnapCenter VMware插件实例(部署虚拟设备时使用的文件名 `.ova` )，然后选择*Delete from Disk*。
